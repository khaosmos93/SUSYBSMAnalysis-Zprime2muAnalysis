...
class Zprime2muHistosNM1FromPAT : public edm::EDAnalyzer {
 public:
  explicit Zprime2muHistosNM1FromPAT(const edm::ParameterSet&);
...
Zprime2muHistosNM1FromPAT::Zprime2muHistosNM1FromPAT(const edm::ParameterSet& cfg)
    ...
    hardInteraction(fill_gen_info ? new HardInteraction(cfg.getParameter<edm::ParameterSet>("hardInteraction")) : 0)
{
  ...
  NVertices = fs->make<TH1F>("NVertices", titlePrefix + "# vertices/event",  200, 0, 200);
  ...
  // For N-1
  LeptonPt_b = fs->make<TH1F>("LeptonPt_b", titlePrefix + "pT B", 5000, 0, 5000);
  LeptonPt_e = fs->make<TH1F>("LeptonPt_e", titlePrefix + "pT E", 5000, 0, 5000);
  LeptonEta_b = fs->make<TH1F>("LeptonEta_b", titlePrefix + "#eta B", 100, -5, 5);
  LeptonEta_e = fs->make<TH1F>("LeptonEta_e", titlePrefix + "#eta E", 100, -5, 5);
  LeptonPhi_b = fs->make<TH1F>("LeptonPhi_b", titlePrefix + "#phi B", 100, -TMath::Pi(), TMath::Pi());
  LeptonPhi_e = fs->make<TH1F>("LeptonPhi_e", titlePrefix + "#phi E", 100, -TMath::Pi(), TMath::Pi());

  ////GenMass = fs->make<TH1F>("GenMass", titlePrefix + "Gen Mass", 20000, 0, 20000);
...
double Zprime2muHistosNM1FromPAT::getSmearedMass(const pat::CompositeCandidate& dil, double gM){
...
void Zprime2muHistosNM1FromPAT::getBSandPV(const edm::Event& event) {
...
void Zprime2muHistosNM1FromPAT::fillBasicLeptonHistos(const reco::CandidateBaseRef& lep) {
  ...
  // For N-1
  if( fabs(lep->eta()) < 1.2 ) { // B
    LeptonPt_b->Fill(lep->pt(), _madgraphWeight*_kFactor);
    LeptonEta_b->Fill(lep->eta(), _madgraphWeight*_kFactor);
    LeptonPhi_b->Fill(lep->phi(), _madgraphWeight*_kFactor);
  }
  else {
    LeptonPt_e->Fill(lep->pt(), _madgraphWeight*_kFactor);
    LeptonEta_e->Fill(lep->eta(), _madgraphWeight*_kFactor);
    LeptonPhi_e->Fill(lep->phi(), _madgraphWeight*_kFactor);
  }
...
void Zprime2muHistosNM1FromPAT::fillOfflineMuonHistos(const pat::Muon* mu) {
...
void Zprime2muHistosNM1FromPAT::fillOfflineElectronHistos(const pat::Electron* lep) {
  // Can add electron quantities here.
}

void Zprime2muHistosNM1FromPAT::fillLeptonHistos(const reco::CandidateBaseRef& lep) {
...
void Zprime2muHistosNM1FromPAT::fillLeptonHistos(const edm::View<reco::Candidate>& leptons) {
...
void Zprime2muHistosNM1FromPAT::fillLeptonHistosFromDileptons(const pat::CompositeCandidateCollection& dileptons) {
  int nleptons = 0;
  int total_q = 0;
...
void Zprime2muHistosNM1FromPAT::fillDileptonHistos(const pat::CompositeCandidate& dil, const edm::Event& event, double gM) {
...
void Zprime2muHistosNM1FromPAT::fillDileptonHistos(const pat::CompositeCandidateCollection& dileptons, const edm::Event& event, double gM) {
...
void Zprime2muHistosNM1FromPAT::analyze(const edm::Event& event, const edm::EventSetup& setup) {
  if (dbg_tree) {
  ...
  if (!hardInteraction->IsValid() )
    edm::LogWarning("HistosNM1FromPAT") << "!hardInteraction->IsValid()";
  else {
    GenM = hardInteraction->resonance->mass();
    GenMass->Fill(GenM, _madgraphWeight*_kFactor);
  }
  */
...
DEFINE_FWK_MODULE(Zprime2muHistosNM1FromPAT);
